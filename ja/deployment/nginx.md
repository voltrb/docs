...
もし Volt を直接起動したい場合は、Nginx の背後に配置して動かすことを推奨します。([bundle exec volt precompile](deployment/README.md) によって) Nginx が静的なアセットをサーブするように設定することが可能です。以下の nginx.conf を利用できます (後述する mime.types ファイルを含めるのを忘れないようにしてください) 。
...
次の mime.types は、新しいフォントフォーマットをサポートするための更新を含むものです。
